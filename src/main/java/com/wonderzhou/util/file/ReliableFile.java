/**
 * copy from org.eclipse.osgi.framework.internal.reliablefile.ReliableFile
 */
...
/**
 * ReliableFile class used by ReliableFileInputStream and ReliableOutputStream. This class encapsulates all the logic
 * for reliable file support.
 * 
 */
public class ReliableFile {
    /**
     * Open mask. Obtain the best data stream available. If the primary data contents are invalid (corrupt, missing,
     * etc.), the data for a prior version may be used. An IOException will be thrown if a valid data content can not be
     * determined. This is mutually exclusive with <code>OPEN_FAIL_ON_PRIMARY</code>.
     */
    public static final int OPEN_BEST_AVAILABLE = 0;
    /**
     * Open mask. Obtain only the data stream for the primary file where any other version will not be valid. This
     * should be used for data streams that are managed as a group as a prior contents may not match the other group
     * data. If the primary data is not invalid, a IOException will be thrown. This is mutually exclusive with
     * <code>OPEN_BEST_AVAILABLE</code>.
     */
    ...
    /**
     * Property to set the maximum size of a file that will be buffered. When calculating a ReliableFile checksum, if
     * the file is this size or small, ReliableFile will read the file contents into a <code>BufferedInputStream</code>
     * and reset the buffer to avoid having to read the data from the media twice. Since this method require memory for
     * storage, it is limited to this size. The default maximum is 128-KBytes.
     */
    public static final String PROP_MAX_BUFFER = "osgi.reliableFile.maxInputStreamBuffer"; //$NON-NLS-1$
    /**
     * The maximum number of generations to keep as backup files in case last generation file is determined to be
     * invalid.
     */
    ...
    /**
     * ReliableFile object factory. This method is called by ReliableFileInputStream and ReliableFileOutputStream to get
     * a ReliableFile object for a target file. If the object is in the cache, the cached copy is returned. Otherwise a
     * new ReliableFile object is created and returned. The use count of the returned ReliableFile object is
     * incremented.
     * 
     ...
     * This call only returns if a file exists and not if the file contents are valid.
     * 
     * @param file returns true if the specified reliable file exists; otherwise false is returned
     * 
     * @return <code>true</code> if the specified reliable file exists, <code>false</code> otherwise.
     */
    ...
    /**
     * Returns the time that this ReliableFile was last modified. This method is only valid after requesting an input
     * stream and the time of the actual input file is returned.
     * 
     * @return time the file was last modified (see java.io.File.lastModified()) or 0L if an input stream is not open.
     */
    ...
    /**
     * Returns the a version number of a reliable managed file. The version can be expected to be unique for each
     * successful file update.
     * 
     * @param file the file to determine the version of.
     * @return a unique version of this current file. A value of -1 indicates the file does not exist or an error
     *         occurred.
     */
     ...
     * 
     * @return <code>true</code> if the specified reliable file was deleted, <code>false</code> otherwise.
     */
    ...
    /**
     * Get a list of ReliableFile base names in a given directory. Only files with a valid ReliableFile generation are
     * included.
     * 
    ...
    /**
     * Returns the size of the ReliableFile signature + CRC at the end of the file. This method should be called only
     * after calling getInputStream() or getOutputStream() methods.
     * 
     * @return <code>int</code> size of the ReliableFIle signature + CRC appended to the end of the file.
     * @throws IOException if getInputStream() or getOutputStream has not been called.
     */
    ...
    /**
     * Returns a Checksum object for the current file contents. This method should be called only after calling
     * getInputStream() or getOutputStream() methods.
     * 
     * @return Object implementing Checksum interface initialized to the current file contents.
     ...
     * 
     * @return Object implementing Checksum interface used to calculate a reliable file checksum
     */
